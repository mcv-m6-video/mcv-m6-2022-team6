...
{
	int hfsize = (__maxx(hfilter.width(),hfilter.height())-1)/2;
	int vfsize = (__maxx(vfilter.width(),vfilter.height())-1)/2;
		...
		switch(type){
			...
			case collapse_max:
				...
				for(int j=1;j<nChannels;j++)
					data[i] = __maxx(data[i],pData[offset+j]);
				break;
			case collapse_min:
				data[i] = pData[offset];
				for(int j = 1;j<nChannels;j++)
...
void Image<T>::threshold()
{
	...
	for(int i = 0;i<nPixels*nChannels;i++)
		pData[i] = __min(__maxx(pData[i],0),ImgMax);
}
																		...
																		const Image<T2>& vx,const Image<T2>& vy) const
{
	...
	for(int i  = 0; i<height; i++)
		for(int j = 0;j<width;j++)
		{
			...
			int x1 = x0+1;
			int y1 = y0+1;
			x0 = __min(__maxx(x0,0),imWidth-1);
			x1 = __min(__maxx(x1,0),imWidth-1);
			y0 = __min(__maxx(y0,0),imHeight-1);
			y1 = __min(__maxx(y1,0),imHeight-1);
			...
			for(int k = 0;k<nChannels;k++)
			{
				...
				//output.pData[offset*nChannels+k] = __maxx(__min(output.pData[offset*nChannels+k],ImgMax),0);
