...
//                  Max = __maxx(Max,abs(pImagePlane[i]));
//          else
//              for(int i=0;i<nElements;i++)
//                  Max = __maxx(Max,fabs((double)pImagePlane[i]));
//          Max*=2;
//          break;
//      case normalized:
...
//                      else
//                          im.data[offset2+j] = __maxx(__min(pImagePlane[offset1+j],255),0);
//                      break;
//                  case derivative:
//
//                      if(IsFloat)
...
bool ImageIO::writeImage(const QString& filename, const T*& pImagePlane,int width,int height,int nchannels,ImageType type,int quality)
{
    ...
    switch(type){
        ...
        case derivative:
            ...
            {
                if(IsFloat)
                    _Max=__maxx(_Max,fabs((double)pImagePlane[i]));
                else
                    _Max=__maxx(_Max,abs(pImagePlane[i]));
            }
            break;
        case normalized:
            ...
            for(int i=1;i<nElements;i++)
            {
                ...
                _Max=__maxx(_Max,pImagePlane[i]);
            }
            break;
    }
...
unsigned char ImageIO::convertPixel(const T& value,bool IsFloat,ImageType type,T& _Max,T& _Min)
{
    switch(type){
        case standard:
            if(IsFloat)
                return __maxx(__min(value*255,255),0);
            else
                return __maxx(__min(value,255),0);
            break;
        case derivative:
