...
def bboxes_size_filter(det_bboxes):
    filtered_bboxes = []
    for b in det_bboxes:
...
def eval_tracking(num_frames, gt, det, accumulator, cam):
    ...
    for frame_id in tqdm(range(num_frames), desc='Evaluating Cam ' + cam):
        ...
        det_bboxes = []
        if frame_id in det:
            det_bboxes = bboxes_size_filter(det[frame_id])
...
def eval_mtmc(reid_path, data_path):
    ...
    return mh.compute(accumulator, metrics=['idf1', 'idp', 'idr', 'precision', 'recall'], name='acc')
...
if __name__ == "__main__":
    ...
    mh = mm.metrics.create()
    summary = mh.compute(accumulator, metrics=['idf1', 'idp', 'idr', 'precision', 'recall'], name='acc')
    print(summary)